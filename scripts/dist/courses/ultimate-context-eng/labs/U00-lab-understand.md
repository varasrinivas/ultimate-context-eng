...
**Expected output (your replay numbers match the shipped fixtures):**
```
NAIVE                    token receipt
System                               65
Retrieved                         1,612
...
✓ VERIFIED
```
(If the frontend is running WITHOUT the backend — `VITE_MOCK=1` — you'll see the mock's larger illustrative numbers instead; the audit rule below holds either way.)
...
Q1's correct answer needs exactly three criteria names and three weights — roughly 40 tokens of facts. Divide: facts ÷ input total ≈ 40 ÷ 1,694 ≈ **2.4%** — and naive mode is being KIND here, because replay's naive dump is only this repo's small seed. The rest is what Anthropic calls spending your **attention budget** on low-signal tokens; on a production data set the ratio falls under 1%.
...
2. The facts-to-input ratio was ~2.4% on this small seed (and falls under 1% at production scale). What ratio — and what correctness bar — would convince you a context strategy "works"?
